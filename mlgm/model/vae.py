...
class Vae(Model):
    def __init__(self, encoder_layers, decoder_layers, latent_dim, sess,
            name="vae"):
        self._encoder = Model(encoder_layers, sess, model_name="encoder")
        self._decoder = Model(decoder_layers, sess, model_name="decoder")
        self._name = name

    def _encode(self, layer_in):
        mean, logvar = tf.split(self._encoder.build_forward_pass(layer_in),
    ...
    def _decode(self, z):
        logits = self._decoder.build_forward_pass(z)
        return logits
    ...
    def build_forward_pass(self, layer_in, use_tensors=None):
        self._mean_sym, self._logvar_sym = self._encode(layer_in)
        self._latent_sym = self._reparameterize(
                self._mean_sym, self._logvar_sym)
        return self._decode(self._latent_sym)
    ...
    def _log_normal_pdf(self, sample, mean, logvar, raxis=1):
        ...
        return tf.reduce_sum(np.float64(-.5)
    ...
    def build_loss(self, labels, logits):
        ...
        cross_ent = tf.nn.sigmoid_cross_entropy_with_logits(logits=logits,
                labels=labels)
