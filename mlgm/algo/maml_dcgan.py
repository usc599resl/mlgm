...
class MamlDcGan:
    ...
    def _build_train(self):
        with self._sess.graph.as_default():
            ...
            train_loss_a, train_losses_b, _ = self._build(
                input_a,
                label_a,
                input_b,
                label_b,
    ...
    def _compute_pretrain(self):
        loss_a, _ = self._sess.run([self._train_loss_a, self._pretrain_op])
        return loss_a
    ...
    def _compute_gen_metatrain(self, itr):
        loss_a, losses_b, _ = self._sess.run([
            self._g_train_loss_a, self._g_train_losses_b,
    ...
    def _test_metalearner(self):
        g_loss_a, g_losses_b, d_loss_a, d_losses_b, output = self._sess.run([
            self._g_test_loss_a, self._g_test_losses_b, self._d_test_loss_a,
            self._d_test_losses_b, self._output
        ])
        return g_loss_a, g_losses_b, d_loss_a, d_losses_b, output
    ...
    def train(self, restore_model_path=None):
        ...
        for i in range(self._metatrain_itr):
            ...
            self._metasampler.restart_test_dataset(self._sess)
            g_loss_a, g_losses_b, d_loss_a, d_losses_b, output = \
