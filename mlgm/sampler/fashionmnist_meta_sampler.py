...
class FashionMnistMetaSampler(MetaSampler):
    def __init__(
        ...
        assert train_classes is None or (
            type(train_classes) == list
            and [0 <= c <= 9 for c in train_classes])

        assert test_classes is None or (
            type(test_classes) == list
            and [0 <= c <= 9 for c in test_classes])
        ...
        for c in self._train_classes:
            ids = np.where(c == labels)[0]
            random.shuffle(ids)
            self._train_inputs_per_label.update({c: ids})

        for c in self._test_classes:
            ids = np.where(c == labels)[0]
            random.shuffle(ids)
            self._test_inputs_per_label.update({c: ids})
    ...
    def _gen_dataset(self, test=False):
        ...
        classes = self._test_classes if test else self._train_classes
        ...
        while True:
            ...
            if tasks_remaining <= 0:
                break            
            tasks_to_add = list(permutations(classes, self._num_classes_per_batch))
